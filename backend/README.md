...
- `pm_fake_visa` - Visa card
- `pm_fake_mastercard` - Mastercard
- `pm_fake_amex` - American Express
- `pm_fake_discover` - Discover

### Valid Currencies
- `USD`, `EUR`, `GBP`, `CAD`, `AUD`, `JPY`

### Force Failure
Add `?force=failure` query parameter to confirmation endpoint to guarantee failure.
...
- Same key returns identical response (status, headers, body)
- Prevents duplicate charges and operations

### Async Processing
- Simulated with `setTimeout` and random delays
- 90% success rate unless forced to fail
